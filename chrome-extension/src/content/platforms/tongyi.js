// ChatSavor — Tongyi platform module

(function() {
  'use strict';

  if (location.hostname !== 'tongyi.aliyun.com' && location.hostname.slice(-18) !== '.tongyi.aliyun.com') return;

  var core = window.ChatSavorCore;
  if (!core) return;

  console.log('[ChatSavor] Tongyi module loaded');
  ...
  var STOP_TOKENS = ['停止生成', '停止回答', '停止输出', 'stop generating', 'stop response'];
    ...
    for (var i = 0; i < RESPONSE_SELECTORS.length; i++) {
      try {
        ...
        if (matched.length > 0) {
          ...
          console.log('[ChatSavor] Tongyi selector:', RESPONSE_SELECTORS[i], '(' + matched.length + ' blocks)');
          return matched;
        }
      } catch (e) {}
    }
