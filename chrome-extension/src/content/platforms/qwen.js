// ChatSavor — Qwen platform module

(function() {
  'use strict';

  if (location.hostname !== 'qwen.ai' && location.hostname.slice(-8) !== '.qwen.ai') return;

  var core = window.ChatSavorCore;
  if (!core) return;

  console.log('[ChatSavor] Qwen module loaded');
    ...
    for (var i = 0; i < RESPONSE_SELECTORS.length; i++) {
      try {
        ...
        if (matched.length > 0) {
          ...
          console.log('[ChatSavor] Qwen selector:', RESPONSE_SELECTORS[i], '(' + matched.length + ' blocks)');
          return matched;
        }
      } catch (e) {}
    }
